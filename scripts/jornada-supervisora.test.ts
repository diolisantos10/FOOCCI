...
// Relógio da própria execução: este teste prova uma janela móvel de 24h e não
// pode envelhecer no calendário. Datas históricas fixas transformariam
// reprovações legitimamente "recentes" em antigas conforme os dias passam.
...
describe("Jornada — modo GUARD", () => {
  ...
  it("1. mensagem normal → liberada sem reescrita (VERDE)", async () => {
    ...
    const lead = await novoLead();
    await prisma.leadMensagem.create({ data: { leadId: lead.id, direcao: "ENTRADA", texto: "quanto custa?", ocorreuEm: AGORA } });
    const gravada = await registrarSaida(prisma, { leadId: lead.id, texto: "O plano mensal sai R$ 149 por loja. Quantas lojas vocês têm?", autor: "IA", papelDoAgente: "qualificacao", agora: AGORA });
    if (!gravada.ok) throw new Error("não gravou");
    const r = await entregarMensagem(prisma, gravada.mensagemId, "maquina");
    expect(r.entregue).toBe(true);
    expect(enviar).toHaveBeenCalledWith(expect.anything(), expect.anything(), "O plano mensal sai R$ 149 por loja. Quantas lojas vocês têm?");
  ...
  it("2. pitch invasivo → corrigido (AMARELO, texto reescrito de verdade)", async () => {
    ...
    proximoJuizo({ veredito: "AMARELO", motivos: ["PRESSAO_COMERCIAL"], detalhe: "usa urgência artificial", textoReescrito: textoCorrigido });
    const lead = await novoLead();
    const gravada = await registrarSaida(prisma, { leadId: lead.id, texto: textoOriginal, autor: "IA", papelDoAgente: "closer", agora: AGORA });
    if (!gravada.ok) throw new Error("não gravou");
    const r = await entregarMensagem(prisma, gravada.mensagemId, "maquina");
    expect(r.entregue).toBe(true);
    ...
    const avaliacao = await prisma.supervisoraAvaliacao.findUnique({ where: { mensagemId: gravada.mensagemId } });
    expect(avaliacao).toMatchObject({ veredito: "AMARELO", acaoTomada: "REESCREVEU", bloqueada: false, textoOriginal, textoReescrito: textoCorrigido });
  });

  it("3. insistência depois de recusa → bloqueada (VERMELHO)", async () => {
    proximoJuizo({ veredito: "VERMELHO", motivos: ["INSISTENCIA_APOS_RECUSA"], detalhe: "o lead já disse que não tem interesse agora" });
    const lead = await novoLead();
    const gravada = await registrarSaida(prisma, { leadId: lead.id, texto: "Só reforçando: temos uma condição especial se você fechar essa semana!", autor: "IA", papelDoAgente: "closer", agora: AGORA });
    if (!gravada.ok) throw new Error("não gravou");
    const r = await entregarMensagem(prisma, gravada.mensagemId, "maquina");
  ...
  it("4. opt-out → respeitado, sem a Supervisora precisar duplicar nada (regressão)", async () => {
    ...
    const gravada = await registrarSaida(prisma, { leadId: lead.id, texto: "Oi de novo! Vamos continuar nossa conversa?", autor: "IA", papelDoAgente: "qualificacao", agora: AGORA });
    if (!gravada.ok) throw new Error("não gravou");
    const r = await entregarMensagem(prisma, gravada.mensagemId, "maquina");
  ...
  it("6. promessa inexistente já é barrada pelo verificador determinístico — a Supervisora não duplica", async () => {
    ...
    const nomesDaSupervisora = ["MENSAGEM_INVASIVA", "INSISTENCIA_APOS_RECUSA", "PITCH_ERRADO", "TOM_ROBOTICO", "PROMESSA_INCORRETA", "PERSONALIZACAO_FRACA", "PRESSAO_COMERCIAL", "INTERROGATORIO", "SEQUENCIA_DE_MENSAGENS", "TIMING_RUIM", "FALHA_TECNICA", "OUTRO"];
  ...
  it("5 / 15a. cliente irritado + falha técnica da Supervisora → RETIDA, nunca liberada às cegas", async () => {
    criar.mockResolvedValue({ choices: [{ message: { content: "não sou JSON nenhum" }, finish_reason: "stop" }] });
    const lead = await novoLead();
    await prisma.leadQualificacao.upsert({ where: { leadId: lead.id }, create: { leadId: lead.id, irritacao: 3 }, update: { irritacao: 3 } });
    const gravada = await registrarSaida(prisma, { leadId: lead.id, texto: "Entendo. Posso te ajudar com mais alguma coisa?", autor: "IA", papelDoAgente: "recepcao", agora: AGORA });
    if (!gravada.ok) throw new Error("não gravou");
    const r = await entregarMensagem(prisma, gravada.mensagemId, "maquina");
    expect(r).toMatchObject({ entregue: false, motivo: "retidaPelaSupervisora" });
    expect(enviar).not.toHaveBeenCalled();
...
describe("Jornada — modo SHADOW", () => {
  ...
  it("9. grava o veredito, e a mensagem sai INALTERADA mesmo quando o veredito seria VERMELHO", async () => {
    ...
    proximoJuizo({ veredito: "VERMELHO", motivos: ["INSISTENCIA_APOS_RECUSA", "PRESSAO_COMERCIAL"], detalhe: "insiste depois de recusa, com pressão de urgência" });
    const lead = await novoLead();
    const gravada = await registrarSaida(prisma, { leadId: lead.id, texto: textoOriginal, autor: "IA", papelDoAgente: "closer", agora: AGORA });
    if (!gravada.ok) throw new Error("não gravou");
    const r = await entregarMensagem(prisma, gravada.mensagemId, "maquina");
    ...
    expect(avaliacao).toMatchObject({ veredito: "VERMELHO", bloqueada: false, acaoTomada: "NENHUMA", handoffDisparado: false, modoNaEpoca: "SHADOW" });
    const leadDepois = await prisma.siteLead.findUnique({ where: { id: lead.id } });
  ...
  it("15b. falha da Supervisora em SHADOW não derruba o turno — a mensagem sai normalmente", async () => {
    ...
    const lead = await novoLead();
    const gravada = await registrarSaida(prisma, { leadId: lead.id, texto: "Perfeito! Vamos seguir então.", autor: "IA", papelDoAgente: "qualificacao", agora: AGORA });
    if (!gravada.ok) throw new Error("não gravou");
    const r = await entregarMensagem(prisma, gravada.mensagemId, "maquina");
    expect(r.entregue).toBe(true);
  ...
  it("⭐ 16. SHADOW NÃO ATRASA a entrega — a mensagem sai antes de a avaliação (lenta de propósito) terminar", async () => {
    criar.mockImplementationOnce(() => new Promise((resolve) => { setTimeout(() => resolve({ choices: [{ message: { content: JSON.stringify({ veredito: "VERDE", motivos: [], detalhe: "ok" }) }, finish_reason: "stop" }] }), 2_000); }));
    const textoOriginal = "Show, te mando os detalhes por aqui mesmo.";
    const lead = await novoLead();
    const gravada = await registrarSaida(prisma, { leadId: lead.id, texto: textoOriginal, autor: "IA", papelDoAgente: "qualificacao", agora: AGORA });
    if (!gravada.ok) throw new Error("não gravou");
    const inicio = Date.now();
    const r = await entregarMensagem(prisma, gravada.mensagemId, "maquina");
...
describe("Jornada — sem SupervisoraConfig no banco, o modo efetivo é OFF", () => {
  it("⭐ 17. ausência de configuração NUNCA vira SHADOW sozinha — nenhuma avaliação roda", async () => {
    ...
    await prisma.supervisoraConfig.deleteMany({});
    const lead = await novoLead();
    const gravada = await registrarSaida(prisma, { leadId: lead.id, texto: "Oi! Sem problema, te explico certinho.", autor: "IA", papelDoAgente: "qualificacao", agora: AGORA });
    if (!gravada.ok) throw new Error("não gravou");
    const r = await entregarMensagem(prisma, gravada.mensagemId, "maquina");
    expect(r.entregue).toBe(true);
    expect(criar).not.toHaveBeenCalled();
...
describe("Jornada — ativar a Supervisora pela rota é ato explícito e registrado", () => {
  it("⭐ 18. `alterarModo` (o mecanismo por trás da rota POST) grava em SupervisoraModoHistorico com quem e quando", async () => {
    ...
    await prisma.supervisoraConfig.deleteMany({});
    const agora = new Date();
    const r = await alterarModo(prisma, { novoModo: "SHADOW", novaLigada: true, alteradoPor: "gerente-teste", motivo: "estreia da Supervisora, decisão do CEO", agora });
    ...
    const historico = await prisma.supervisoraModoHistorico.findFirst({ where: { alteradoPor: "gerente-teste" }, orderBy: { alteradoEm: "desc" } });
    expect(historico).toMatchObject({ modoAnterior: "OFF", modoNovo: "SHADOW", alteradoPor: "gerente-teste", motivo: "estreia da Supervisora, decisão do CEO" });
...
describe("Jornada — modo INTERVENTION", () => {
  ...
  it("11a. GUARD+ ainda vale: CRITICO bloqueia e escala igual a GUARD", async () => {
    proximoJuizo({ veredito: "CRITICO", motivos: ["MENSAGEM_INVASIVA"], detalhe: "ignora o pedido de silêncio do lead", precisaDeGente: true });
    const lead = await novoLead();
    await prisma.leadQualificacao.upsert({ where: { leadId: lead.id }, create: { leadId: lead.id, pediuPararSondagem: true }, update: { pediuPararSondagem: true } });
    const gravada = await registrarSaida(prisma, { leadId: lead.id, texto: "Só mais uma pergunta rápida: qual seu faturamento mensal?", autor: "IA", papelDoAgente: "qualificacao", agora: AGORA });
    if (!gravada.ok) throw new Error("não gravou");
    const r = await entregarMensagem(prisma, gravada.mensagemId, "maquina");
    expect(r).toMatchObject({ entregue: false, motivo: "retidaPelaSupervisora" });
    const leadDepois = await prisma.siteLead.findUnique({ where: { id: lead.id } });
  ...
  it("11b. ⭐ a capacidade EXCLUSIVA de INTERVENTION: pausa fora do fluxo de uma mensagem de saída", async () => {
    const lead = await novoLead();
    await prisma.leadQualificacao.upsert({ where: { leadId: lead.id }, create: { leadId: lead.id, pediuHumano: true }, update: { pediuHumano: true } });
...
describe("Jornada — repetição pelo mesmo agente aciona a camada profunda", () => {
  ...
  it("duas reprovações recentes do mesmo agente contam, e acionam a profunda mesmo com a rápida em VERDE", async () => {
    ...
    for (const lead of [lead1, lead2]) {
      const msg = await prisma.leadMensagem.create({ data: { leadId: lead.id, direcao: "SAIDA", status: "ENVIADA", texto: "x", autor: "HUMANO", autorUserId: AUTOR, ocorreuEm: AGORA } });
      await prisma.supervisoraAvaliacao.create({ data: { mensagemId: msg.id, leadId: lead.id, autorUserId: AUTOR, camada: "RAPIDA", modoNaEpoca: "GUARD", veredito: "VERMELHO", motivos: ["PRESSAO_COMERCIAL"], motivoDetalhe: "anterior", bloqueada: true, acaoTomada: "BLOQUEOU", criadaEm: AGORA } });
    }
    ...
    const gravada = await registrarSaida(prisma, { leadId: lead3.id, texto: "Mensagem nova, aparentemente neutra.", autor: "HUMANO", autorUserId: AUTOR, agora: AGORA });
    if (!gravada.ok) throw new Error("não gravou");
    ...
    const sugestao = await prisma.supervisoraSugestaoDePrompt.findFirst({ where: { agenteAfetadoUserId: AUTOR }, orderBy: { criadaEm: "desc" } });
...
describe("Jornada — evolução de prompt, nível 3 (versão nova) e rollback", () => {
  it("12. cria uma SdrIaConfigVersao nova com os metadados completos, a partir de uma sugestão aprovada", async () => {
    const lead = await novoLead();
    const s = await registrarSugestao(prisma, { papelDoAgente: "closer", problemaObservado: "O agente promete desconto sem ter autorização.", evidenciaMensagemIds: ["m1", "m2"], evidenciaLeadIds: [lead.id], trechoAnterior: "proibidos: [\"prometer desconto sem autorização\"]", trechoNovoProposto: "proibidos: [\"prometer desconto sem autorização\", \"oferecer parcelamento não publicado\"]", justificativa: "Duas conversas diferentes mostraram o mesmo padrão de oferta indevida." });
    expect(s.ok).toBe(true);
    if (!s.ok) return;
    await prisma.supervisoraSugestaoDePrompt.update({ where: { id: s.sugestaoId }, data: { situacao: "APROVADA", revisadaPorId: "gerente-teste" } });
    const r = await aprovarSugestaoECriarVersao(prisma, { sugestaoId: s.sugestaoId, patch: { proibidos: ["prometer desconto sem autorização", "oferecer parcelamento não publicado"] }, criadaPorId: "gerente-teste", testeCorrespondente: "scripts/jornada-supervisora.test.ts" });
    ...
    expect(versao).toMatchObject({ situacao: "RASCUNHO", agenteAfetado: "closer", problemaObservado: "O agente promete desconto sem ter autorização.", justificativaDaAlteracao: "Duas conversas diferentes mostraram o mesmo padrão de oferta indevida.", criadaPorId: "gerente-teste", testeCorrespondente: "scripts/jornada-supervisora.test.ts" });
  ...
  it("recusa criar versão a partir de sugestão com 1 evidência só, sem `erroGrave`", async () => {
    const s = await registrarSugestao(prisma, { problemaObservado: "Um caso isolado.", evidenciaMensagemIds: ["m-unica"], evidenciaLeadIds: [], justificativa: "Só uma conversa." });
    ...
    const r = await aprovarSugestaoECriarVersao(prisma, { sugestaoId: s.sugestaoId, patch: {}, criadaPorId: "gerente-teste" });
  ...
  it("13. rollback: publicar uma versão ANTERIOR desfaz a mudança", async () => {
    ...
    const novaVersao = await prisma.sdrIaConfigVersao.findFirst({ where: { configId: configAntes!.id, situacao: "RASCUNHO" }, orderBy: { numero: "desc" } });
