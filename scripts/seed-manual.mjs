 ...
 * seed-manual.mjs
 *
 * Seeds the initial chapter structure for the Foocci Operational Manual.
 * Idempotent — safe to run multiple times.
 *
 * Run: node --env-file=.env.local scripts/seed-manual.mjs [--dry-run]
 *
 * This script does NOT connect agents or enable agent visibility.
...
async function main() {
  console.log(`\n📖  Foocci Operational Manual — seed inicial`);
  ...
  for (const ch of CHAPTERS) {
    ...
    if (exists) {
      console.log(`   ⏭  Skip  — "${ch.title}" (já existe)`);
      skipped++;
      continue;
    }

    if (dryRun) {
      console.log(`   🔍  DryRun — "${ch.title}" seria criado (área: ${ch.area})`);
      created++;
      continue;
    }
    ...
    console.log(`   ✅  Criado — "${ch.title}" (${ch.area})`);
    created++;
  }

  console.log(`\n   Total: ${created} criados, ${skipped} ignorados.`);
  if (dryRun) console.log("   (dry-run — nenhuma mudança foi gravada)");
  console.log("\n📖  Seed do Manual Operacional concluído.\n");
}
