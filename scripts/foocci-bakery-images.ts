 ...
 *   npm run bakery:imagens -- --yes --limit=5
...
async function main(): Promise<void> {
  ...
  const plano = await planBakeryImages({ regerar: force });
  ...
  const aGerar = limite && limite > 0 ? Math.min(limite, plano.aGerar) : plano.aGerar;

  console.log(`    itens sem foto : ${plano.semFoto}${force ? " (--force: inclui quem já tem)" : ""}`);
  console.log(`    serão gerados  : ${aGerar}`);
  console.log(`    modelo         : ${plano.modelo}`);
  console.log(`    custo estimado : ~US$ ${(aGerar * plano.custoPorFotoUsd).toFixed(2)}\n`);
  ...
  const r = await generateBakeryImages({
    ...
    regerar: force,
    limite,
    onProgress: ({ indice, total, item }) => {
      process.stdout.write(`  [${indice}/${total}] ${item} …\n`);
    },
  });

  console.log(`\n━━━ Resumo ━━━`);
  console.log(`  geradas : ${r.geradas}`);
  console.log(`  puladas : ${r.puladas} (já tinham foto quando chegou a vez — nada gasto)`);
