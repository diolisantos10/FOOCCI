...
const SMOKE: WaOrderingScenario[] = [
  {
    ...
    expectedFinalStage: ["COLLECTING_DELIVERY_TYPE", "COLLECTING_REQUIRED_OPTIONS", "REVIEWING_ORDER", "BUILDING_CART"],
    menuDependent: true,
...
const FULL: WaOrderingScenario[] = [
  ...
  },
  {
    ...
    messages: ["quero um yakisoba de camarão", "9", "retirada", "cartão"],
    suites: ["full", "payment"],
...
const EDGE: WaOrderingScenario[] = [
  ...
  },
  {
    ...
    category: "basic",
    description: "Vários itens e instruções na mesma mensagem — itens, entrega e Pix capturados; mostra comanda e aguarda confirmação. Nunca volta para 'o que deseja'.",
    messages: ["2 yakisoba frango, 1 coca normal, entrega, pix"],
    suites: ["edge"],
    expectedItems: [{ name: "yakisoba", quantity: 2 }, { name: "coca", quantity: 1 }],
    expectedDeliveryType: "DELIVERY",
    expectedPaymentMethod: "PIX",
    expectedFinalStage: "BUILDING_CART",
    menuDependent: true,
    tags: ["mixed"],
    ...SAFE,
  },
