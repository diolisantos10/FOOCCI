...
export const WAITER_EVAL_CRITERIA: readonly EvalCriterion[] = [
  ...
  { label: "Não despeja o cardápio inteiro", automated: true, check: "no_long_menu_dump", note: "Teto = 12 cards (alinhado à recomendação ampla)." },
  { label: "Quantidade de cards por contexto (amplo 8–12 · contextual 3–6)", automated: true, check: "no_long_menu_dump", note: "v1.5: intenção ampla mostra 8–12; pairing/checkout ficam 3–6." },
  ...
  { label: "Restrição: cauteloso, sem afirmar dieta sem tag confiável", automated: true, check: "no_hallucination", note: "v1.5: handler determinístico (vegano/sem peixe…) — nunca diz 'não temos', confirma ingredientes." },
...
export const WAITER_INTELLIGENCE_BACKLOG: readonly BacklogItem[] = [
  ...
  {
    id: "p0-restriction",
    title: "Restrição alimentar sem tag confiável (MITIGADO v1.5)",
    problem: "Sem metadata de dieta, o Waiter não pode afirmar 'é vegano' nem 'não temos'.",
    impact: "Risco de sugerir item incompatível OU negar opções que existem.",
    where: "WaiterBrainV2.handleRestriction (determinístico, cauteloso) + testes",
    risk: "MEDIUM",
    nextStep: "Mitigado (v1.5): handler cauteloso (vegano/vegetariano/sem peixe/porco/alergia) que exclui carne/peixe aparente, nunca crava dieta e pede confirmação de ingredientes. PRÓXIMO: metadata/tags confiáveis de dieta por item.",
    phase: "Fase 2 — Test Center",
    priority: "P0",
  },
  {
  ...
  {
    id: "p1-three-cards",
    title: "Mais cards em intenção ampla (MITIGADO v1.5)",
    problem: "Recomendações amplas mostravam poucos cards (limite 5).",
    impact: "Menos opções = menor descoberta e ticket médio.",
    where: "WaiterBrainV2 suggest() + RECOMMENDATION_CARD_CAP + waiterEvaluator",
    risk: "LOW",
    nextStep: "Mitigado (v1.5): intenção ampla mostra 8–12 cards; contextual (pairing/checkout) fica 3–6; teto do evaluator alinhado a 12. PRÓXIMO: ordenar por venda real quando houver telemetria.",
    phase: "Fase 2/3",
    priority: "P1",
  },
  {
