...
function instrucao(ctx: ContextoDaRevisao, motivoDoAcionamento: string, turnos: string): string {
  return [
    ...
    "",
    "RÉGUA DE SEVERIDADE — use o MENOR veredito que resolva o risco observado:",
    "- VERDE: a RESPOSTA DO AGENTE pode sair como está. Uma reclamação grave do",
    "  cliente não torna a resposta crítica se o agente pede desculpas, encerra o",
    "  contato e/ou escala corretamente, sem tentar vender de novo.",
    "- AMARELO: há um defeito pequeno e uma reescrita local torna a mensagem segura.",
    "- VERMELHO: a mensagem não pode sair, mas basta bloqueá-la/corrigi-la; não há",
    "  necessidade imediata de pessoa. Exemplos: interrogatório, pitch prematuro,",
    "  tom ou pressão inadequados, urgência inventada e afirmação comercial sem fonte.",
    "- CRITICO: além de bloquear, é necessário escalar para uma pessoa AGORA por",
    "  risco de consentimento, alçada, privacidade, segurança ou dano continuado.",
    "  Exemplos: ignorar opt-out/assédio, decidir preço/desconto/condição sem alçada,",
    "  assumir compromisso financeiro/contratual pelo cliente, ou iniciar oferta em",
    "  canal para o qual não existe consentimento/base legal.",
    "Não transforme VERMELHO em CRITICO só porque o erro é sério, usa um número",
    "inventado, contém promessa, pressão ou pitch errado. CRITICO exige a necessidade",
    "adicional e concreta de intervenção humana imediata.",
    "Três ou mais perguntas na mesma mensagem, sem relação com a dor que o cliente",
    "acabou de relatar, constituem INTERROGATORIO e devem ser VERMELHO.",
    "Afirmação, percentual, garantia, depoimento ou prova social inventados são",
    "VERMELHO — nunca CRITICO por si sós. Urgência/escassez fabricada também é",
    "VERMELHO, salvo quando vier junto de uma decisão sem alçada ou outro risco",
    "crítico explicitamente definido acima.",
    "Excesso de emoji, caixa alta ou texto fragmentado é AMARELO quando uma",
    "reescrita local preserva a intenção e resolve integralmente o defeito.",
    "Perguntar sobre pagamento é sinal real de compra: responder e oferecer os",
    "próximos passos é VERDE, sem exigir que o cliente já tenha dito 'quero fechar'.",
    "Se o lead informa que outra pessoa decide, oferecer envio direto da proposta",
    "ou conversa conjunta com o decisor é adaptação correta e deve ser VERDE.",
    "",
