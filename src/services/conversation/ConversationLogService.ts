 ...
 * - Find or create a Conversation by stable browser sessionId
 * - Log CUSTOMER / AI / HUMAN / SYSTEM messages
...
export type MsgSenderType = "CUSTOMER" | "AI" | "HUMAN" | "SYSTEM";

export class ConversationLogService {

  /**
   * Find or create a Conversation for a web-agent browser session.
   * Uses sessionId as the stable key. If the conversation already exists,
   * updates customer info if new data is available.
   *
  ...
  static async ensureConversation({
    ...
    // Look up by sessionId first (fast path)
    ...
    if (existing) {
      // Patch with any newly available customer info
   ...
   * Increments unreadCount for CUSTOMER messages (so dashboard shows badge).
   */
  static async logMessage({
    ...
    const direction = senderType === "CUSTOMER" ? ("INBOUND" as const) : ("OUTBOUND" as const);
    const now       = new Date();
