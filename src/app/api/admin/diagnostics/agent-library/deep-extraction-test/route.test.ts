...
function req() { return {} as unknown as Parameters<typeof POST>[0]; }

const OLD = process.env.ADMIN_SECRET;
beforeEach(() => {
  vi.clearAllMocks();
  process.env.ADMIN_SECRET = "test-secret";
  runDiag.mockResolvedValue({ ok: true, status: "PASS", techniquesCreated: 5, sourceCleanedUp: true, runtimeTouched: false });
});
afterEach(() => { process.env.ADMIN_SECRET = OLD; });

describe("POST /api/admin/diagnostics/agent-library/deep-extraction-test", () => {
  it("(10) requires admin auth — 401, does not run the diagnostic", async () => {
