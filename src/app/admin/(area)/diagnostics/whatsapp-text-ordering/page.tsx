...
interface RoutingReadiness {
  ...
  input: { phone: string; phoneMasked: string; message?: string | null };
  flags: {
...
export default function WaTextOrderingPage() {
  ...
  // Sample message — drives the full message-aware routing contract.
  const [rdMessage, setRdMessage] = useState("Bom dia");

  const checkRouting = useCallback(async (overrideMessage?: string) => {
    const message = overrideMessage ?? rdMessage;
    setRdLoading(true); setRdError(null);
    try {
      const res = await fetch("/api/admin/diagnostics/whatsapp-text-ordering/routing", {
        method:  "POST",
        headers: { "Content-Type": "application/json" },
        body:    JSON.stringify({ restaurantSlug: slug, phone, message }),
      });
      if (!res.ok) {
        const b = await res.json().catch(() => ({}));
        throw new Error(b.error ?? `HTTP ${res.status}`);
    ...
    <div className="mx-auto max-w-4xl px-6 py-8 text-gray-900">
      ...
      <div className="mt-3 rounded-xl border border-blue-200 bg-blue-50 p-4">
        ...
        <p className="mt-1 text-[11px] text-blue-800">
          Usa o restaurante e telefone dos campos abaixo + a mensagem de exemplo. Não envia WhatsApp, não cria pedido
          nem Pix — só responde se a mensagem real entraria no Pedido por Texto ou ficaria com o Agente WhatsApp (host).
        </p>

        {/* Sample message + quick examples — drives the full message-aware contract */}
        <div className="mt-3 flex flex-col gap-2 sm:flex-row sm:items-end">
          <label className="flex flex-1 flex-col gap-1 text-[11px] font-semibold text-blue-900">
            Mensagem de exemplo
            <input
              type="text"
              value={rdMessage}
              onChange={e => setRdMessage(e.target.value)}
              placeholder="Ex.: Bom dia  |  Quero 1 yakisoba"
              className="rounded-md border border-blue-300 px-3 py-2 text-sm font-mono"
            />
          </label>
          <div className="flex flex-wrap gap-1">
            {["Bom dia", "Oi", "Cardápio", "Quero 1 yakisoba"].map(ex => (
              <button
                key={ex}
                onClick={() => { setRdMessage(ex); void checkRouting(ex); }}
                disabled={rdLoading}
                className="rounded-md border border-blue-300 px-2 py-1 text-[11px] font-semibold text-blue-700 hover:bg-blue-100 disabled:opacity-50"
              >
                {ex}
              </button>
            ))}
        ...
        {rdReport && (
          <div className="mt-3 space-y-2 text-xs">
            <div className="flex flex-wrap items-center gap-2">
              ...
              {rdReport.messageAware && (
                <Pill text={`sessão ativa: ${rdReport.messageAware.activeSession ? "sim" : "não"}`} tone={rdReport.messageAware.activeSession ? "green" : "gray"} />
              )}
            ...
            {rdReport.input.message && (
              <p className="text-[11px] text-gray-700">
                Mensagem testada: <span className="font-mono">&quot;{rdReport.input.message}&quot;</span>
              </p>
            )}
            <div className="grid grid-cols-2 gap-1 text-[11px] text-gray-700 sm:grid-cols-3">
              <span>{rdReport.configEligible ?? rdReport.flags.enabledForRestaurant ? "✅" : "⬜"} config elegível (restaurante+telefone)</span>
              <span>{rdReport.messageAware?.messageHasOrderIntent ? "✅" : "⬜"} intenção de pedido</span>
              <span>{rdReport.messageAware?.activeSession ? "✅" : "⬜"} sessão de pedido ativa</span>
            ...
            {!rdReport.wouldRouteToTextOrdering && (
              <div className="rounded-lg border border-amber-300 bg-amber-50 px-3 py-2 text-amber-900">
                ...
                {(rdReport.messageAware?.declineReason ?? rdReport.declineReason) && (
                  <p className="mt-1">Motivo: {rdReport.messageAware?.declineReason ?? rdReport.declineReason}</p>
                )}
      ...
      <div className="mt-3 rounded-xl border border-emerald-200 bg-emerald-50 p-4">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          ...
          <button onClick={() => void checkRouting()} disabled={rdLoading}
