...
import type { CrmEvalReport, CrmScenarioResult, Verdict } from "@/services/crm/testing/crmEvaluator";
...
function buildFilename(slug: string, ranAt: string, ext: "json" | "txt"): string {
  return `crm-test-${slug}-${fmtDateForFilename(ranAt)}.${ext}`;
}
...
function buildJsonExport(report: CrmEvalReport): Record<string, unknown> {
  return {
...
function buildTxtReport(report: CrmEvalReport): string {
  ...
  const lines: string[] = [
    "===========================================",
    `CRM TEST REPORT -- ${report.restaurantName}`,
    "===========================================",
    `Data:     ${dateStr}`,
    `Slug:     ${report.slug}`,
    `Modo:     ${report.mode}`,
    `Score:    ${report.summary.avgScore}%  (PASS ${report.summary.passed} / WARN ${report.summary.warned} / FAIL ${report.summary.failed} de ${report.summary.total})`,
  ...
  if (failed.length === 0 && warned.length === 0) {
    lines.push("- Tudo verde. CRM determinístico aprovado para o nível atual de autonomia.");
  } else {
    if (failed.length > 0) lines.push("- Revisar as falhas P0/P1 acima antes de liberar mais autonomia ao CRM Agent.");
    if (warned.length > 0) lines.push("- Avisos não bloqueiam, mas indicam divergências de tier/plataforma/offer a conferir.");
  }
  lines.push("");
...
function ResultCard({ result }: { result: CrmScenarioResult }) {
...
export default function CrmTestesPage() {
  const [slug, setSlug] = useState("sushi-cazza");
  const [group, setGroup] = useState<CrmScenarioGroupId | "all">("all");
  const [groups] = useState<CrmScenarioGroup[]>(() => getCrmScenarioGroups());
  const [loading, setLoading] = useState(false);
  const [report, setReport] = useState<CrmEvalReport | null>(null);
  ...
  const run = useCallback(async (mode: Mode) => {
    ...
    try {
      const res = await fetch("/api/admin/ai/crm-tests/run", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(body),
      });
      const data = await res.json() as { ok: boolean; report?: CrmEvalReport; error?: string; warnings?: string[] };
    ...
    <div className="p-6 max-w-4xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-400 mt-1">
          Avaliação determinística da lógica do CRM Agent: segurança, segmentação, inteligência, action center,
          rascunho de mensagem, atribuição e review request. Sem LLM. Sem envios. Sem campanhas. Sem efeitos colaterais.
        </p>
      </div>
      ...
      <div className="rounded-lg border border-gray-800 bg-gray-900 p-4 mb-6 space-y-4">
        ...
        <div>
          ...
          <select
            value={group}
            onChange={(e) => setGroup(e.target.value as CrmScenarioGroupId | "all")}
            disabled={loading}
            className="w-full rounded-lg border border-gray-700 bg-gray-800 px-3 py-2 text-sm text-white focus:outline-none focus:ring-1 focus:ring-violet-500"
          >
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <button type="button" onClick={() => run("group")} disabled={loading || group === "all"} className={`${btn} bg-violet-800 text-white hover:bg-violet-700`}>
            Rodar grupo selecionado
          </button>
          <button type="button" onClick={() => run("full")} disabled={loading} className={`${btn} bg-violet-700 text-white hover:bg-violet-600`}>
