 ...
 *   "Máximo de W km"        → distanceMaxKm   (hard block — out_of_range)
 ...
 *
 * distanceMaxKm is a hard block: if distance exceeds this value the resolver
 * returns out_of_range and both quote and finalize must reject the order.
 */
...
export type DeliveryCalculationStatus =
  ...
  | "out_of_range"              // distance known but exceeds distanceMaxKm → block
...
export interface DeliveryResolverInput {
  ...
  deliveryConfig: {
    ...
    distanceMaxKm?:     number | null; // hard outer radius — orders beyond this are blocked
    distanceMaxFee?:    number | null; // price cap (unrelated to distanceMaxKm)
  };
}
...
export async function resolveDeliveryFee(input: DeliveryResolverInput): Promise<DeliveryFeeResult> {
  ...
  if (deliveryType === "pickup") {
    return mkResult(0, mode, null, null, 0, 0, 0, null, null, false, "pickup", "Retirada — sem taxa de entrega");
  }

  // Manual mode — operator agrees fee with customer
  if (mode === "manual") {
    return mkResult(0, mode, null, null, 0, 0, 0, null, null, false, "manual", "Taxa manual — a combinar com o restaurante");
  }
  ...
  if (freeAbove != null && freeAbove > 0 && subtotal >= freeAbove) {
    return mkResult(0, mode, null, null, 0, 0, 0, null, null, true, "free_delivery",
  ...
  if (mode === "simple") {
    ...
    return mkResult(fee, mode, null, null, fee, 0, 0, null, null, false, "simple", "Taxa fixa de entrega");
  }
  ...
  if (mode === "distance") {
    ...
    const maxDistanceKm = deliveryConfig.distanceMaxKm      != null ? Number(deliveryConfig.distanceMaxKm)      : null;
    ...
    // Distance known — check max radius before calculating fee
    if (distanceKm != null) {
      if (maxDistanceKm != null && distanceKm > maxDistanceKm) {
        console.warn("[delivery-fee] OUT_OF_RANGE: customer exceeds max delivery radius", {
          distanceKm, maxDistanceKm, baseFee, pricePerKm,
        });
        return mkResult(0, mode, distanceKm, maxDistanceKm, baseFee, includedKm, pricePerKm, fallbackSafeFee, maxFee, false,
          "out_of_range",
          `Esse endereço está fora da área de entrega. (${distanceKm.toFixed(1)} km — máximo ${maxDistanceKm.toFixed(1)} km)`);
      }

      const fee = calcDeliveryFeeFromConfig(cfg, distanceKm);
      console.info("[delivery-fee] distance calculated", { baseFee, includedKm, pricePerKm, distanceKm, maxDistanceKm, fee });
      return mkResult(fee, mode, distanceKm, maxDistanceKm, baseFee, includedKm, pricePerKm, fallbackSafeFee, maxFee, false,
        "distance_calculated",
    ...
    if (fallbackSafeFee != null && fallbackSafeFee > 0) {
      ...
      return mkResult(fallbackSafeFee, mode, null, maxDistanceKm, baseFee, includedKm, pricePerKm, fallbackSafeFee, maxFee, false,
    ...
    return mkResult(0, mode, null, maxDistanceKm, baseFee, includedKm, pricePerKm, null, maxFee, false,
      "distance_blocked",
      "Não conseguimos calcular a entrega para esse endereço. Confira o endereço ou fale com o restaurante.");
  }

  // Unknown / advanced mode
  return mkResult(0, mode, null, null, 0, 0, 0, null, null, false, "error", `Modo de entrega não reconhecido: ${mode}`);
}

function mkResult(
  deliveryFee: number, mode: string, distanceKm: number | null, maxDistanceKm: number | null,
  ...
  return { deliveryFee, mode, distanceKm, maxDistanceKm, baseFee, includedKm, pricePerKm, minFee, maxFee, freeDeliveryApplied, calculationStatus, reason };
}
